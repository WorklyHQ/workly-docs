...
- Persistance SQLite dans data/memory/workly.db
...
class EmotionMemory:
    ...
    def __init__(
        ...
        # Cache en mémoire (deque pour performance)
        self.history: deque[EmotionEntry] = deque(maxlen=max_entries)

        # Charger historique existant depuis SQLite
        self._load_history()

    def _load_history(self) -> None:
        """Charge l'historique depuis SQLite"""
        try:
            # Récupérer émotions depuis DB (limit = max_entries)
            emotions_db = self.db.get_emotions(limit=self.max_entries)

            # Reconstruire deque depuis DB (ordre chronologique, plus anciennes d'abord)
            # Note: get_emotions retourne DESC, on doit inverser pour avoir ordre chronologique
            emotions_db.reverse()

            for emotion_data in emotions_db:
                # Convertir dict DB → EmotionEntry
                entry = EmotionEntry(
                    emotion=emotion_data["emotion"],
                    intensity=emotion_data["intensity"],
                    confidence=emotion_data["confidence"],
                    source=emotion_data["source"],
                    message_preview=emotion_data["message_preview"],
                    timestamp=emotion_data["timestamp"],
                    context={"db_id": emotion_data["id"]},  # Garder ID pour référence
                )
                self.history.append(entry)

            if len(self.history) > 0:
                print(f"✅ Chargé {len(self.history)} émotions depuis SQLite")
    ...
    def _save_history(self) -> None:
        """
        Sauvegarde l'historique (No-op avec SQLite)

        Note: Avec SQLite, les émotions sont écrites directement dans add_emotion().
        Cette méthode est gardée pour backward compatibility mais ne fait rien.
        Le cache self.history est déjà à jour.
        """
    ...
    def add_emotion(
        ...
        # Écrire dans SQLite
        emotion_id = self.db.add_emotion(
            emotion=emotion,
            intensity=intensity,
        ...
        entry = EmotionEntry(
            ...
            message_preview=preview,
            timestamp=timestamp,
            context={"db_id": emotion_id, **(context or {})},
        )

        # Ajouter au cache (deque gère max_entries automatiquement)
        self.history.append(entry)

    def get_recent_emotions(
        self, count: int = 10, source: Optional[str] = None
    ) -> List[EmotionEntry]:
        """
    ...
    def get_emotion_distribution(
        self, source: Optional[str] = None, hours: Optional[int] = None
    ) -> Dict[str, int]:
        """
    ...
    def get_emotional_trend(self, window_size: int = 5) -> str:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_intensity_assistant": self.get_average_intensity(
                source="assistant"
            ),
...
if __name__ == "__main__":
    ...
    for i, entry in enumerate(recent, 1):
        print(
            f"   {i}. [{entry.source}] {entry.emotion} ({entry.intensity:.0f}%) - {entry.message_preview[:40]}..."
        )
    print()
