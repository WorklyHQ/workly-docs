...
class WorklyDatabase:
    ...
    def set_personality_trait(
        self, trait_name: str, score: float, description: str = ""
    ) -> int:
        ...
        if row:
            ...
            cursor.execute(
                """
                INSERT INTO personality_evolution (trait_name, old_score, new_score, timestamp)
                VALUES (?, ?, ?, ?)
            """,
                (trait_name, old_score, score, timestamp),
            )
        else:
            # Insert
            cursor.execute(
