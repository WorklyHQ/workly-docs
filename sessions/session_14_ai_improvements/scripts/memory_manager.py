...
- Stockage persistant (JSON) de conversations, faits, embeddings
...
Architecture :
- Conversations stockées en segments (max 30 messages/segment)
...
class MemoryManager:
    ...
    def __init__(
        ...
        Args:
            storage_dir: Dossier de stockage des fichiers JSON
        ...
        if SENTENCE_TRANSFORMERS_AVAILABLE:
            ...
            print("⚠️ sentence-transformers non disponible - recherche sémantique désactivée")
        
        # Charger données existantes
        self.conversations = self._load_json(self.conversations_file, default={'segments': []})
        ...
        self.embeddings_data = self._load_json(self.embeddings_file, default={'embeddings': []})
    ...
    def force_segment_creation(self) -> Dict[str, Any]:
        ...
        self._auto_summarize_and_segment()
        return self.conversations['segments'][-1] if self.conversations['segments'] else {}
    ...
    def _add_or_update_entity(self, entity: Dict[str, Any]) -> None:
        ...
        for existing in self.facts['entities']:
            if (existing['entity_type'] == entity['entity_type'] and 
                existing['value'].lower() == entity['value'].lower()):
                # Incrémenter occurrences
                existing['occurrences'] += 1
                existing['last_seen'] = entity.get('first_seen', datetime.utcnow().isoformat())
                return
    ...
    def _generate_and_store_embedding(
        ...
        embedding_entry = {
            ...
            'embedding': embedding.tolist(),  # Convertir numpy → list pour JSON
            'segment_id': segment_id,
            'metadata': metadata or {},
            'created_at': datetime.utcnow().isoformat()
        }
        
        self.embeddings_data['embeddings'].append(embedding_entry)
        self._save_json(self.embeddings_file, self.embeddings_data)
    ...
    def search_relevant_context(
        ...
        """
        if not self.embedding_model or not self.embeddings_data['embeddings']:
        ...
        results = []
        for entry in self.embeddings_data['embeddings']:
            ...
            if relevant_segments:
                ...
                for seg_data in relevant_segments:
                    ...
                    if segment and segment.get('summary'):
                        formatted = self.summarizer.format_summary_for_context(segment['summary'])
    ...
    def _format_relevant_facts(self, query: str) -> str:
        ...
        if prefs:
            ...
            for pref in prefs[-5:]:  # 5 dernières
                ...
                facts_text.append(f"  - {sentiment} {pref['subject']} ({pref['category']})")
        
        # Entités fréquentes
        entities = self.facts.get('entities', [])
        if entities:
            # Trier par occurrences
            top_entities = sorted(entities, key=lambda e: e.get('occurrences', 1), reverse=True)[:5]
            facts_text.append("\nEntités mentionnées :")
            for ent in top_entities:
                facts_text.append(f"  - {ent['value']} ({ent['entity_type']}, {ent.get('occurrences', 1)}x)")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'embeddings_count': len(self.embeddings_data.get('embeddings', [])),
...
if __name__ == "__main__":
    ...
    manager.add_message('assistant', "C'est génial ! Qu'est-ce que tu aimes particulièrement ?")
    manager.add_message('user', "Les libraries comme numpy et pandas sont super utiles.")
