"""
Emotion Analyzer Avancé - Workly (Kira)
...
# Import EmotionMemory pour historique persistant
try:
...
class EmotionAnalyzer:
    ...
    # Émotions composées (combinaisons d'émotions primaires)
    COMPOUND_EMOTIONS = {
        "excited": {  # Excité = joie + surprise
            "components": {"joy": 0.6, "surprised": 0.4},
            "keywords": ["excité", "impatient", "hâte", "🤩", "trop bien"],
            "description": "Enthousiasme intense avec anticipation"
        },
        "frustrated": {  # Frustré = colère + tristesse
            "components": {"angry": 0.5, "sorrow": 0.5},
            "keywords": ["frustré", "frustrant", "énervant", "😤", "pff"],
            "description": "Irritation mêlée de déception"
        },
        "bittersweet": {  # Doux-amer = joie + tristesse
            "components": {"joy": 0.5, "sorrow": 0.5},
            "keywords": ["nostalgique", "nostalgique", "mélancolique mais heureux"],
            "description": "Mélange de bonheur et de tristesse"
        },
        "anxious": {  # Anxieux = peur + surprise négative
            "components": {"sorrow": 0.6, "surprised": 0.4},
            "keywords": ["anxieux", "inquiet", "stressé", "angoisse", "😰", "😟"],
            "description": "Préoccupation avec incertitude"
        }
    }
    ...
    def __init__(
        ...
        Args:
            ...
            enable_emotion_memory: Activer mémoire émotionnelle persistante
        """
        self.smoothing_factor = max(0.0, min(1.0, smoothing_factor))
        self.history_size = history_size

        # Historique émotionnel par utilisateur (en mémoire, court terme)
        # Format : {user_id: deque([EmotionResult, ...])}
        self.emotion_history: Dict[str, deque] = {}
        
        # Mémoire émotionnelle persistante (long terme)
        self.emotion_memory: Optional[EmotionMemory] = None
        if enable_emotion_memory:
            self.emotion_memory = EmotionMemory()

        logger.info(
            f"✅ EmotionAnalyzer initialisé "
            f"(smoothing={self.smoothing_factor}, history={self.history_size}, "
            f"memory={enable_emotion_memory})"
        )

    def _calculate_intensity(
        self, text: str, keywords_with_weights: Dict[str, int]
    ...
    def _calculate_confidence(
        self, intensity: float, keywords_count: int, context_score: float
    ) -> float:
        """
        ...
        confidence = (
            intensity_factor * 0.4 + keyword_factor * 0.3 + context_factor * 0.3
        ) * 100

        return min(100, confidence)

    def _calculate_context_score(self, current_emotion: str, user_id: str) -> float:
        """
    ...
    def _apply_smoothing(
        self, current_result: EmotionResult, user_id: str
    ) -> EmotionResult:
        """
        ...
        """
        if (
            user_id not in self.emotion_history
            or len(self.emotion_history[user_id]) == 0
        ):
    ...
    def _detect_compound_emotion(
        self,
        text: str,
        primary_emotions: Dict[str, float]
    ) -> Optional[Tuple[str, float]]:
        """
        Détecte une émotion composée dans le texte
        
        Args:
            text: Texte à analyser
            primary_emotions: Scores des émotions primaires détectées
            
        Returns:
            (nom_émotion_composée, score) ou None
        """
        text_lower = text.lower()
        
        for compound_name, compound_data in self.COMPOUND_EMOTIONS.items():
            # Vérifier keywords spécifiques
            has_keyword = any(kw in text_lower for kw in compound_data["keywords"])
            
            if has_keyword:
                # Calculer score basé sur composantes
                component_score = 0.0
                for component, weight in compound_data["components"].items():
                    component_score += primary_emotions.get(component, 0.0) * weight
                
                # Bonus si keyword présent
                total_score = (component_score + 30) * 1.2  # Boost pour keyword direct
                
                return (compound_name, min(100, total_score))
            
            # Vérifier si composantes présentes sans keyword
            components_present = all(
                primary_emotions.get(comp, 0) > 20
                for comp in compound_data["components"].keys()
            )
            
            if components_present:
                # Score basé uniquement sur composantes
                component_score = sum(
                    primary_emotions.get(comp, 0) * weight
                    for comp, weight in compound_data["components"].items()
                )
                
                return (compound_name, min(100, component_score))
    ...
    def analyze(
        ...
        - Détection émotions composées (excited, frustrated, etc.)
        - Intégration EmotionMemory pour historique persistant
        - Analyse contextuelle avancée avec tendances

        Args:
            text: Texte à analyser (réponse du bot ou message utilisateur)
            user_id: ID utilisateur pour historique émotionnel
            context: Contexte conversationnel (optionnel)
            source: 'assistant' ou 'user' (pour EmotionMemory)
        ...
        if not text or not text.strip():
            ...
            # Sauvegarder dans mémoire si activée
            if self.emotion_memory:
                self.emotion_memory.add_emotion(
                    "neutral", 0.0, 100.0, source, text
                )
            return result

        # 1. Analyser chaque émotion primaire et calculer scores
        emotion_scores = {}
        emotion_details = {}

        for emotion, keywords_with_weights in self.EMOTION_KEYWORDS.items():
            intensity, keywords_found = self._calculate_intensity(
                text, keywords_with_weights
            )

            if intensity > 0:
                context_score = self._calculate_context_score(emotion, user_id)
                confidence = self._calculate_confidence(
                    intensity, len(keywords_found), context_score
                )
        ...
        # 2. Vérifier émotions composées
        compound_result = self._detect_compound_emotion(text, emotion_scores)
        
        if compound_result:
            compound_name, compound_score = compound_result
            
            # Si score composé > émotion primaire dominante, utiliser composée
            max_primary_score = max(emotion_scores.values()) if emotion_scores else 0
            
            if compound_score > max_primary_score * 0.8:  # Seuil 80%
                logger.info(f"🎭 Émotion composée détectée : {compound_name} ({compound_score:.1f})")
                
                # Utiliser score composé
                dominant_emotion = compound_name
                details = {
                    "intensity": compound_score,
                    "confidence": 85.0,  # Confiance modérée pour composée
                    "keywords": self.COMPOUND_EMOTIONS[compound_name]["keywords"],
                    "context_score": 70.0
                }
            else:
            ...
            # 3. Déterminer émotion primaire dominante
        ...
        # 6. Sauvegarder dans historique court terme
        self._save_to_history(user_id, result)
        
        # 7. Sauvegarder dans mémoire long terme si activée
        if self.emotion_memory:
            self.emotion_memory.add_emotion(
                result.emotion,
                result.intensity,
    ...
    def get_vrm_blendshape(self, emotion: str, intensity: float) -> Dict[str, Any]:
        """
    ...
    def get_emotional_context(self, user_id: str = "desktop_user") -> str:
        """
        Génère un contexte émotionnel pour injection dans prompt
        
        Utilise EmotionMemory pour analyse approfondie
        
        Args:
            user_id: ID utilisateur
            
        Returns:
            Texte décrivant l'état émotionnel récent
        """
        if not self.emotion_memory:
            return ""
        
        return self.emotion_memory.get_context_for_prompt()
    
    def should_adjust_response_tone(
        self,
        user_id: str = "desktop_user"
    ) -> Optional[str]:
        """
        Détermine si le ton des réponses devrait être ajusté
        
        Args:
            user_id: ID utilisateur
            
        Returns:
            Suggestion d'ajustement ou None
            - 'more_empathetic' : Utilisateur triste/en colère répété
            - 'more_cheerful' : Utilisateur neutre prolongé, manque d'émotion
            - 'calmer' : Utilisateur surexcité/intensité trop élevée
            - None : Ton actuel approprié
        """
        if not self.emotion_memory:
            return None
        
        # Analyser tendance émotionnelle
        trend = self.emotion_memory.get_emotional_trend()
        
        # Émotion dominante utilisateur
        dominant = self.emotion_memory.get_dominant_emotion(
            source='user',
            hours=24
        )
        
        # Intensité moyenne
        avg_intensity = self.emotion_memory.get_average_intensity(
            source='user',
            hours=24
        )
        
        # Patterns spécifiques
        has_consecutive_sad = self.emotion_memory.detect_emotional_pattern(
            'consecutive',
            'sorrow',
            3
        )
        
        has_consecutive_angry = self.emotion_memory.detect_emotional_pattern(
            'consecutive',
            'angry',
            3
        )
        ...
        # Décisions
        if has_consecutive_sad or has_consecutive_angry:
            return 'more_empathetic'
        
        if dominant == 'neutral' and avg_intensity < 30:
            return 'more_cheerful'
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        # Répartition des émotions (court terme)
        emotion_counts = {}
        for history in self.emotion_history.values():
            for result in history:
                emotion_counts[result.emotion] = (
                    emotion_counts.get(result.emotion, 0) + 1
                )

        stats = {
            "total_users": total_users,
            "total_emotions_analyzed": total_emotions,
            "emotion_distribution_short_term": emotion_counts,
            "smoothing_factor": self.smoothing_factor,
            "history_size": self.history_size,
            "emotion_memory_enabled": self.emotion_memory is not None
        }
        
        # Ajouter stats mémoire long terme si activée
        if self.emotion_memory:
            memory_stats = self.emotion_memory.get_statistics()
            stats["emotion_memory_stats"] = memory_stats
...
def get_emotion_analyzer(
    smoothing_factor: float = 0.3, history_size: int = 5
) -> EmotionAnalyzer:
    """
