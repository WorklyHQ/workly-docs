...
Architecture :
- Traits stockés en JSON avec scores 0.0-1.0
...
class PersonalityEngine:
    ...
    def _load_personality(self) -> Dict[str, PersonalityTrait]:
        """
        Charge la personnalité depuis JSON ou initialise par défaut
            ...
            try:
                ...
                for trait_name, trait_data in data.get('traits', {}).items():
                    personality[trait_name] = PersonalityTrait(
                        ...
                        last_updated=trait_data.get('last_updated', datetime.utcnow().isoformat()),
                        evolution_history=trait_data.get('evolution_history', [])
                    )

                return personality

            except Exception as e:
                print(f"⚠️ Erreur chargement personnalité : {e}")
    ...
    def _save_personality(self) -> None:
        """Sauvegarde la personnalité en JSON"""
        try:
            data = {
                'traits': {
                    name: trait.to_dict()
            ...
            print(f"⚠️ Erreur sauvegarde personnalité : {e}")
    ...
    def update_trait(
        ...
        trait_name: str,
        delta: float,
        reason: str = "Évolution naturelle"
    ) -> None:
        """
    ...
    def analyze_user_feedback(
        ...
        positive_indicators = ['merci', 'génial', 'super', 'excellent', 'parfait', 'cool', 'top']
    ...
    def generate_personality_prompt(self) -> str:
        ...
        # Construire phrase
        if traits:
            return f"Tu es {', '.join(traits[:3])}. " + (', '.join(traits[3:]) + '.' if len(traits) > 3 else '')
    ...
    def get_evolution_history(self, trait_name: str, limit: int = 10) -> List[Dict[str, Any]]:
        """
...
if __name__ == "__main__":
    ...
    for entry in history[-3:]:
        if 'new_score' in entry:
            print(f"   {entry['timestamp'][:19]} : {entry.get('old_score', 0):.2f} → {entry['new_score']:.2f}")
        else:
            print(f"   {entry['timestamp'][:19]} : Initialisation à {entry['score']:.2f}")
