...
from PySide6.QtWidgets import (
    ...
    QTextEdit, QLineEdit, QInputDialog, QListWidget, QListWidgetItem,
...
class DiscordBotThread(QThread):
    ...
    def __init__(self, token: str, parent: Optional[QObject] = None):
        """
        ...
        try:
            ...
            async def wrapped_on_ready():
                ...
                self.signals.status_changed.emit(
                    status['connected'],
                    status.get('bot_name', 'Bot Discord')
                )
...
class MainWindow(QMainWindow):
    ...
    def start_discord_bot(self):
        ...
        if self.discord_running:
            QMessageBox.information(
                self,
                "Bot Déjà Actif",
                "Le bot Discord est déjà en cours d'exécution."
            )
            return
        ...
        # Connect signals
        self.discord_thread.signals.status_changed.connect(self.on_discord_status_changed)
        self.discord_thread.signals.message_received.connect(self.on_discord_message_received)
    ...
    def manage_auto_reply_channels(self):
        ...
        remove_btn.clicked.connect(lambda: self._remove_channel_from_list(channels_list))
    ...
    def _add_channel_to_list(self, list_widget: QListWidget):
        ...
        channel_id, ok = QInputDialog.getText(
            self,
            "Ajouter Salon Discord",
            "Entrez l'ID du salon Discord :"
        )
    ...
    def _remove_channel_from_list(self, list_widget: QListWidget):
        ...
        if current_item:
            ...
            QMessageBox.information(
                self,
                "Aucune Sélection",
                "Veuillez sélectionner un salon à retirer."
            )
    ...
    def _save_channels(self, list_widget: QListWidget, dialog: QDialog):
        ...
        logger.info(f"✅ Salons auto-reply sauvegardés : {len(auto_reply_channels)} salons")
